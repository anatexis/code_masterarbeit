...
Q <- ggplot(data= discharge)+
  ...
  xlab("Date")+
  ylab("Discharge [mm]")+
  annotate("text", x=as.Date(15600), y=30,label="nse= ")+
  annotate("text", x=as.Date(16000), y=30,label=as.character(round(nse,3)))+
  annotate("text", x=as.Date(15600), y=28,label="kge= ")+
  annotate("text", x=as.Date(16000), y=28,label=as.character(round(kge,3)))+
  scale_color_manual(values=c("Qobs"="#00BFC4", "Qsim"="#F8766D",
                              "lin"="#7CAE00", "casc"="#C77CFF"))
  
Q
setwd("C:/Users/Russ/Desktop/master/plotfiles_Hofstetten/VAL_plots_daily/")
...
file.copy(file,"C:/Users/Russ/Desktop/master/plotfiles_Hofstetten/VAL_plots_daily/")
