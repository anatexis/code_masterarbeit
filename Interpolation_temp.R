...
pst2 <- pst2[as_date(pst2$date) < as_date("2017-09-05"), ]
pst3 <- pst3[as_date(pst3$date) < as_date("2017-09-05"), ]
pst4 <- pst4[as_date(pst4$date) < as_date("2017-09-05"), ]


# put all P-data in one data-frame

pst1 <- add_column(pst2,pst3$Temp,pst4$Temp)
pst1 <- rename(pst1, c(Temp="N107300",'pst3$Temp'="LT107466",
                       'pst4$Temp'="LT115642"))
############ koord
...
xy <- filter(xy, ID %in% c("N115642","N107300","N107466")) # not that good
#xy <- filter(xy, start == 1971) # we dont want all 1971s

###________________________________________________lles löschen__________________________________________________________
